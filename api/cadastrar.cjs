...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // 4. Montagem dos dados para inserção (USANDO NOMES DE COLUNAS MINÚSCULOS)
        const cadastroData = {
            nome_completo: data.nome_completo || data.nome,
            email: data.email, // Corrigido para minúsculo
            ...
            bairro: data.bairro, // Corrigido para minúsculo
            cidade: data.cidade,
            estado: data.estado,
        ...
        if (error) {
            ...
            if (error.code === '23505') { // Erro de Chave Única/Duplicidade
